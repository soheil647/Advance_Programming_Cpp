...
void Hotel::get_new_rating(float location, float cleanliness, float staff, float facilities, float value_for_money, float overall_Rating) {
    ratings.push_back(new Ratings(this->unique_id, location, cleanliness, staff, facilities, value_for_money, overall_Rating));
}
...
float Hotel::find_average_price(){
    ...
    if(number_of_room_type == 0)
        return 0;
    float average = sum_of_price/float(number_of_room_type);
    return average;
}
...
void Hotel::print_hotel() {
    ...
    cout << "#rooms: " << number_standard_room << " " << number_deluxe_room << " " << number_luxury_room << " " << number_premium_room << endl;
    cout << "price: " << standard_room_price << " " << deluxe_room_price << " " << luxury_room_price << " " << premium_room_price << endl;
}
...
void Hotel::save_rating(const std::vector<std::string>& rating_information) {
    ...
    this->file_rating = new Ratings(unique_info, _location_rating, _cleanliness_rating, _staff_rating, _facilities_rating, _value_for_money_rating, _overall_rating);
}

float Hotel::get_rating(const std::vector<Ratings *>& user_ratings, RatingWeights *user_rating_weights) {
