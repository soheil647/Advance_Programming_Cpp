...
private:
    ...
    RatingWeights* estimated_weights;
    ...
    static double find_diffrantiate(Ratings *const &user_rate, std::vector<double> weights, int count);

    static double find_error(double y_hat, double y);

    static double cut_float(double number, double high, double low);
public:
