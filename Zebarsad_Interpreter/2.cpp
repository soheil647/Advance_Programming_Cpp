...
vector<vector<std::__cxx11::string>> interPreter::parse_line(const vector<string> &input_line) {
...
interPreter::Operators interPreter::resolve_operator(const string &new_operator) {
    if (new_operator == "+") return Add;
    else if (new_operator == "-") return Subtract;
    else if (new_operator == "/") return Divide;
    else if (new_operator == "*") return Multiply;
